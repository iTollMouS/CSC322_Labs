...
// Tyler Debrino
{
    ...
    public static int m = /*CSC*/ 322;
    public static int n = m;
    public static Point3f fun = new Point3f(0,0,0);
    ...
    private static void renderUnitSphere() {
        ...
        // drawing points to make the sphere
        glEnable(GL_POINT_SMOOTH);
        glBegin(GL_POINTS);

        float points[][][] = fun.pointCloudPolarSphere(m, n);
        // front x face
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                glVertex3f(points[i][j][0], points[i][j][1], points[i][j][2]);//sets all of the points to the calculates coordinates
            }
        }
        for (int i = 0; i < m-1; i++)
        {
            for(int j=0;j<n-1;j++)
            {
                glNormal3f(points[i][j][1]*points[i+1][j+1][2]-points[i][j][2]*points[i+1][j+1][1],points[i][j][2]*points[i+1][j+1][0]-points[i][j][0]*points[i+1][j+1][2],points[i][j][0]*points[i+1][j+1][1]-points[i][j][1]*points[i+1][j+1][0]);
            }
        }
